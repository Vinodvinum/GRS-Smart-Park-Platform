...
export default function OperationsPage() {
  ...
  useEffect(() => {
    loadSnapshot().catch(() => setNotice("Operations data could not be loaded."));
  }, [loadSnapshot]);
  ...
  async function assign(code: string) {
    ...
    if (!response.ok) {
      setNotice("The request could not be assigned.");
      return;
    }
    await loadSnapshot();
      ...
      <main className="container opsContent">
        <div className="opsHeader">
          <div>
            ...
            <p>Operational workspace · Database-backed snapshot</p>
          </div>
          <button className="misButton" onClick={() => loadSnapshot().then(() => setNotice("Operational snapshot refreshed.")).catch(() => setNotice("Refresh failed."))}>
        ...
        <div className="opsPanels">
          <section className="opsPanel">
            <div className="panelTitle"><div><div className="kicker">ATTRACTIONS</div><h3>Capacity & queue</h3></div><span>{attractions.length} configured</span></div>
            <div className="tableHead"><span>Attraction</span><span>Capacity</span><span>Queue</span><span>Status</span></div>
            {attractions.length === 0 && <div className="emptyState">No attraction snapshot is available yet.</div>}
            {attractions.map((a) => (
              <div className="zoneRow" key={a.id}>
                <b>{a.name}<small>{a.zone}</small></b>
                <span>{a.capacity}%</span>
                <span>{a.queueMinutes} min</span>
                <span className={`status ${a.status === "OPEN" ? "low" : a.status === "LIMITED" ? "medium" : "high"}`}>{a.status}</span>
              </div>
            ))}
          </section>
          ...
          <section className="opsPanel">
            ...
            {requests.length === 0 && <div className="emptyState">No active service requests.</div>}
            {requests.slice(0, 6).map((r) => (
              <div className="opRequest" key={r.id}>
                <div className="avatar">{r.category[0]}</div>
                <div className="opRequestMain">
                  <b>#{r.requestCode} · {r.category}</b>
                  <small>{r.location || "Location not provided"} · {r.assignedTo ?? "Unassigned"}</small>
                </div>
                <span className={`status ${r.priority === "CRITICAL" ? "high" : r.priority === "HIGH" ? "medium" : "low"}`}>{r.priority}</span>
                {!r.assignedTo && <button className="miniButton" onClick={() => assign(r.requestCode)}>Assign</button>}
              </div>
            ))}
          </section>
        </div>
        ...
        <section className="opsPanel operationsTools">
          ...
          <div className="toolGrid">
            ...
            <Tool icon={<Ticket/>} title="Bookings" text="Review the booking queue." onClick={() => setNotice("Booking management is the next operations workflow.")} />
            ...
            <Tool icon={<AlertTriangle/>} title="Incidents" text="Open the incident workflow." onClick={() => setNotice("Incident management is available to supervisors and admins.")} />
          </div>
        </section>
        ...
        <section className="misCallout">
          ...
          <a href="https://grsdashboard.streamlit.app/" className="misButton" target="_blank" rel="noreferrer">Open MIS <ArrowRight size={14}/></a>
        </section>
      </main>
      ...
      {qrOpen && (
        <div className="modalBack" onMouseDown={(e) => { if (e.target === e.currentTarget) closeQR(); }}>
          <div className="modal" role="dialog" aria-modal="true" aria-labelledby="qr-dialog-title">
            ...
            <p style={{ color: "#667286", fontSize: 11, lineHeight: 1.5 }}>Paste a pass token from a confirmed booking to validate it server-side.</p>
            <label style={{ display: "block", fontSize: 11, fontWeight: 700, marginTop: 20 }} htmlFor="qr-token-input">QR token
              <input id="qr-token-input" value={qrValue} onChange={e => setQrValue(e.target.value)} placeholder="Enter pass token" style={{ display: "block", width: "100%", marginTop: 7, padding: 12, border: "1px solid #e5eaf1", borderRadius: 10 }}/>
            </label>
            <button className="primary" onClick={validateQR} disabled={!qrValue.trim()}>Validate <CheckCircle2 size={15}/></button>
            {qrResult && <div role="status" style={{ marginTop: 15, padding: 14, borderRadius: 12, background: qrResult === "VALID_PASS" ? "#e9f7f1" : "#ffe8e8", color: qrResult === "VALID_PASS" ? "#198a62" : "#bf3c46", fontSize: 12, fontWeight: 800 }}>{qrResult === "VALID_PASS" ? "✓ Valid pass — accepted" : "✕ Invalid pass"}</div>}
          </div>
        </div>
      )}
    </div>
