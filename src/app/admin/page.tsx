...
import { ArrowLeft, ArrowRight, Building2, ChevronRight, ClipboardList, Gift, LayoutDashboard, MapPinned, RefreshCw, Settings, ShieldCheck, Sparkles, Users, Waves } from "lucide-react";
  ...
  return <div className="opsShell">
    ...
    <div className="adminLayout">
      ...
      <main className="adminMain">
        <div className="adminHeader"><div><div className="kicker">GRS PLATFORM ADMIN</div><h1>{title}</h1><p>Configuration and governance · Database-backed platform</p></div><button className="misButton" onClick={() => load().then(() => setNotice("Admin snapshot refreshed.")).catch(() => setNotice("Refresh failed."))}><RefreshCw size={14}/> Refresh</button></div>
...
function AdminPanel({ tab, data }: { tab: string; data: AdminData | null }) {
  ...
  if (tab === "attractions") return <section className="opsPanel adminTable"><TableHead title="Attractions"/><div className="adminTableHead"><span>Name</span><span>Experience</span><span>Status</span></div>{data.attractions.map(a => <div className="adminTableRow" key={a.id}><b>{a.name}</b><span>{a.zone}</span><span className={`status ${a.status === "OPEN" ? "low" : "medium"}`}>{a.status}</span></div>)}</section>;
  return <section className="opsPanel adminTable"><TableHead title="Offers"/><div className="adminTableHead"><span>Name</span><span>Code</span><span>Status</span></div>{data.offers.map(o => <div className="adminTableRow" key={o.id}><b>{o.name}</b><span>{o.type}</span><span className={`status ${o.status === "ACTIVE" ? "low" : "medium"}`}>{o.status}</span></div>)}</section>;
}
